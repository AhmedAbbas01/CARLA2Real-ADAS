...
CARLA RGB Camera -> Fine-tuned YOLOv8 (best.pt) -> Object detections ->
ADAS risk logic -> Throttle / Brake command -> CARLA ego vehicle -> Next
camera frame

YOLO performs perception only. The ADAS logic converts YOLO detections
into vehicle-control commands.
...
The closed-loop script performs inference only. It does not retrain
YOLO.
...
The names must match the class names stored in your trained model.

8. Current Collision-Risk Logic
...
For this straight-line AEB test, CARLA autopilot is disabled:
...
Therefore it is a straight-line AEB experiment, not a complete
...
Confirm that your trained checkpoint exists at the path configured in
MODEL_PATH.
...
This verifies that detector output is feeding back into CARLA vehicle
control.
...
is normal when there is no qualifying detection in the approximate
ego-lane region.
...
Bounding-box size is not true distance

The current system uses bounding-box size as a heuristic. Do not report
bbox_ratio as distance in meters.

No true TTC

The current implementation does not yet calculate physical
Time-To-Collision.

Straight-line test

Autopilot is disabled and steer is fixed at zero. This version validates
longitudinal AEB control first.

Detector errors affect control

False negatives can prevent braking. False positives can cause
unnecessary braking.

Thresholds need validation

WARNING_BOX_RATIO and BRAKE_BOX_RATIO are experimental and should be
...
If you receive an error saying the function is not implemented,
remove/comment:
...
Some CARLA builds do not contain:

    vehicle.tesla.model3

If that produces “RuntimeError: index out of range”, select an available
vehicle from:

    world.get_blueprint_library().filter("vehicle.*")

A robust script can try the Tesla first and fall back to another
vehicle.
...
The cleanup section should stop/destroy the camera and destroy the
spawned ego vehicle.
...
-   Quantitative AEB experiments such as stopping distance and collision
    rate
...
The main final test is to introduce an obstacle and observe the
controller transition from:
...
while confirming that the CARLA ego vehicle physically decelerates and
stops.
